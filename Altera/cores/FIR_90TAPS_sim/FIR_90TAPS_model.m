...
for j = 1:sx
    ...
    %
    file_name = ['FIR_90TAPS_input.txt'];
     ...
     % run this output through the model 
      for i =  1 : num_ch
        if(not(isempty(single_channel_data{i})))
          output_ch{i} = FIR_90TAPS_mlab(single_channel_data{i}, single_channel_bank{i});
  ...
  for j = 1: limit;
    ...
    file_name = ['FIR_90TAPS_model_output'];

    outfile1 = fopen([file_name, '.txt'],'w');
    for i=1:length(out{i})
        if out{i}(i)>=0
         fprintf(outfile1, '%s\n',dec2hex(out{i},11));
        else
         fprintf(outfile1, '%s\n',dec2hex(2^(11*4)+out{i},11));
        end
    end
    fclose(outfile1);
